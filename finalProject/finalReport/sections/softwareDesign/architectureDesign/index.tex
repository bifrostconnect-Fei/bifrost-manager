...
\begin{tikzpicture}[node distance=3.5cm, every node/.style={align=center, minimum width=3cm, minimum height=3cm}]
    % Nodes
    \node (client) [rectangle, draw] {Client\\(Web Browser)};
    \node (server) [rectangle, draw, right of=client] {Server};
    \node (mqtt) [rectangle, draw, below of=server] {MQTT Server};
    \node (database) [rectangle, draw, below of=mqtt] {Database\\(SQLite)};
    ...
    % Additional module
    \node (module) [rectangle, draw, right of=server, xshift=2cm] {Additional Module\\(Remote Communication)};
    \draw[->] (server) -- (module);
    \draw[->] (module) -- (mqtt);
