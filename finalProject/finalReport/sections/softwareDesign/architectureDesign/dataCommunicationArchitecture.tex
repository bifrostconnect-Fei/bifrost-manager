...
% Nodes
\node (frontend) [rectangle, draw, text width=3cm, align=center] {Front-end};
\node (backend) [rectangle, draw, text width=3cm, align=center, right of=frontend] {Back-end};
\node (http) [ellipse, draw, below of=frontend] {HTTP};
\node (webrtc) [ellipse, draw, below of=http] {WebRTC};
\node (mqtt) [ellipse, draw, below of=webrtc] {MQTT};
\node (sqlite) [rectangle, draw, text width=3cm, align=center, right of=backend] {SQLite};

% Arrows
\draw [->] (frontend) -- (http);
\draw [->] (http) -- node[midway, right] {CRUD Requests} (backend);
\draw [->] (frontend) -- (webrtc);
\draw [->] (webrtc) -- node[midway, right] {Streaming Media Data} (backend);
\draw [->] (frontend) -- (mqtt);
\draw [->] (mqtt) -- node[midway, right] {Device Status \& Operation Info} (backend);
\draw [->] (backend) -- (sqlite);
